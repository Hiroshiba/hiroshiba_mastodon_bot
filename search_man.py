...
class SearchMan(StreamListener):
    ...
    def should_respect_ltl(old_status, new_status, min_seconds=10):
        old_time = datetime.datetime.strptime(
            old_status["created_at"], "%Y-%m-%dT%H:%M:%S.%fZ"
        )
        new_time = datetime.datetime.strptime(
            new_status["created_at"], "%Y-%m-%dT%H:%M:%S.%fZ"
        )
    ...
    def make_text_with_page(page, acct):
        ...
        status = (
            "@{acct} ".format(acct=acct)
            + page.content[: 450 - len(page.url)]
            + ".....\n"
            + page.url
        )
        return status, spoiler_text
    ...
    def on_update(self, status):
        ...
        # status history
        if len(self.status_history) >= 1 and self.should_respect_ltl(
            self.status_history[0], status
        ):
